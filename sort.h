...
struct Add {
    using Result = Add< typename T1::template Push_back< T2::head >, typename T2::Tail >::Result;
};
...
struct Filter< F, T, typename std::enable_if_t< !std::is_same< T, int >::value && F< T::head >::value > > {
    using Result = Add< Arr< T::head >, typename Filter< F, typename T::Tail >::Result >::Result;
};

template< template< int > typename F, typename T >
struct Filter< F, T, typename std::enable_if_t< !std::is_same< T, int >::value && !F< T::head >::value > > {
    using Result = Filter< F, typename T::Tail >::Result;
};
...
struct Sort {
    ...
    using Ls = Sort< typename Filter< Lc, T >::Result >::Result;

    template< int X >
    using Ec = typename Cmp< T::head >::template Equal  < X >;
    using Es = Filter< Ec, T >::Result;

    template< int X >
    using Gc = typename Cmp< T::head >::template Greater< X >;
    using Gs = Sort< typename Filter< Gc, T >::Result >::Result;
